...
def story_rewrite(text):
    ...
    try:
        ...
        text_to_process = text[:1000] if len(text) > 1000 else text
...
8. ВАЖНО: Результат должен быть НЕ ДЛИННЕЕ 1500 символов для Discord
...
{text_to_process}"""
        ...
        if len(rewritten_text) > 1950:  # Оставляем запас
            rewritten_text = rewritten_text[:1900] + "...\n\n#ОсколкиГлянца"
